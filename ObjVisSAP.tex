...
of current IVOA Data Access Layer Interface (DALI; \cite{2017ivoa.spec.0517D}) and Observation Data
Model Core Components (ObsCore;\cite{2017ivoa.spec.0509L}) and its implementation in
the Table Access Protocol (TAP;\cite{2019ivoa.spec.0927D}). ObjVisSAP services also support
VOSI-availability and VOSI-capabilities resources (VOSI;\cite{2017ivoa.spec.0524G}):
...
\textbf{T\_MIN }parameter, to specify the position in the sky to check the visibility. 
...
\begin{itemize}
\item Exactly one field \textbf{MAY }have a name="\textbf{sun\_sep\_min}" 
with datatype="double", ucd="angle.validity" and
...
\begin{itemize}
\item Exactly one field \textbf{MAY }have a name="\textbf{sun\_sep\_max}" 
with datatype="double", ucd="angle.validity" and
