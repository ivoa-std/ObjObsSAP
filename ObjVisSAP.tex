...
(\cite{std:SSAP}) and SIAP v2.0 (\cite{std:SIAP}) through the adoption
of current IVOA Data Access Layer Interface (DALI) and Observation Data
Model Core Components (\cite{std:OBSCORE}) and its implementation in
...
ObjVisSAP specifies standardID values \citep{std:VOID2} for each
capability, as defined by VODataService \citep{std:VOR}. ObjVisSAP
services may be registered in an IVOA Registry using the SimpleDALRegExt
\citep{DALREGEXT} extension schema.
...
A web service with ObjVisSAP capabilities  \citep{std:VOSI} must have a
VOSI-availability resource as described in DALI \citep{std:DALI}.
...
The ObsCore data model specifies all the VOTable \citep{std:VOTABLE}
...
The \{query\} response must contain the required ObsCore \citep{std:OBSCORE}
...
RESPONSEFORMAT parameter \citep{std:DALI11}.\par
